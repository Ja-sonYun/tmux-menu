...
function wifi_signal() {
    tmux display-popup -x R -y P -w 75 -h 20 -E \
        "{ while true; do /System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport --getinfo | awk '/agrCtlRSSI/ {print -\$2; fflush();}'; sleep 1; done } | ttyplot -t \"wifi signal\" -u \"-dBm\" -s 90" || true
}
